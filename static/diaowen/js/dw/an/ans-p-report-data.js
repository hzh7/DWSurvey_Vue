...
function parseRadio(item,pageNo){
  ...
  var quCoBottomTools = $("#quCoBottomTools");
  lastQuItemBody.append(quCoBottomTools.html());
  parseChoseReport(lastQuItemBody, item)
  var quInputCase = lastQuItemBody.find(".quInputCase");
...
function parseChoseReport(lastQuItemBody, item){
  var hiddenChoseTag = lastQuItemBody.find(".hidden_chose_tag");
  ...
  $(hiddenChoseTag).change(function (){
    for (let i=0; i<hiddenChoseTag.length; i++) {
      if (hiddenChoseTag[i].checked && hiddenChoseTag[i].value === 'n') {
        lastQuItemBody.find('.hidden_chose_type_div').prop('style', 'display: none')
      } else {
        lastQuItemBody.find('.hidden_chose_type_div').prop('style', 'display: inline')
      }
    }
  })
}
